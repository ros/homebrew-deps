...
#
#   https://github.com/Homebrew/homebrew-core/issues/5725
#
# We've resurrected it here because ROS Indigo still needs it.
# We used this command to get the most recent content:
#
#   git -C "$(brew --repo homebrew/core)" show e7e836c^:Formula/pyside.rb
...
class Pyside < Formula
  ...
  if build.with? "python3"
    depends_on "ros/deps/shiboken" => "with-python3"
  else
    depends_on "ros/deps/shiboken"
  end
  ...
  def install
    ...
    Language::Python.each_python(build) do |python, version|
      ...
      mkdir "macbuild#{version}" do
        qt = Formula["osrf/simulation/qt4-no-webkit"].opt_prefix
    ...
    inreplace include/"PySide/pyside_global.h", Formula["osrf/simulation/qt4-no-webkit"].prefix, Formula["osrf/simulation/qt4-no-webkit"].opt_prefix
  end
